...
export default class extends Vue {
  ...
  async getValidationStatusInfo() {
    ...
    if (data.validatingYn == 'Y' && data.wantToStop != 'Y') {
      this.$emit('onRun','RUN');
    }
  }
  async getHdqrList() {
    ...
    this.selectHdqr = this.hdqrList[0];   // 전체 기능 쓰는 경우 제거
    
    this.getMtnList(this.selectHdqr);
  }
  async getMtnList(hdqr) {
    // this.hdqrList = [];
    // const data = await commonService.request(
    //   {},
    //   "/api/updt/hdqr/list"
    ...
    // });
    // console.log("hdqrList ===",this.hdqrList);    
    // this.selectMtn = this.hdqrList[0];   // 전체 기능 쓰는 경우 제거
    
    //임시
    this.selectMtn = '수원';
    this.getTolList(this.selectMtn);
  }
  async getTolList(mtn) {
    // this.hdqrList = [];
    // const data = await commonService.request(
    //   {},
    //   "/api/updt/hdqr/list"
  ...
  onChangeHdqr(event) {
    this.selectHdqr = event.target.value;
  }
  onChangeMtn(event) {
    this.selectMtn = event.target.value;
  }
  onChangeTol(event) {
    this.selectTol = event.target.value;
  }
  onChangeCrgwTypeCd(event) {
    this.selectCrgwTypeCd = event.target.value;
  }
  onChangeCrgwNo(event) {
    this.selectCrgwNo = event.target.value;
  }
